...
class CustomController(BaseController):
    ...
    def update(self, timestep):
        ...
        psi_desired = np.arctan2(trajectory[closest_index + look_ahead_distance, 1] - Y, trajectory[closest_index + look_ahead_distance, 0] - X)

        # error calculation (referencing Project 2 solution)
        e1 = (Y - trajectory[closest_index + look_ahead_distance,1])*np.cos(psi_desired) - (X - trajectory[closest_index+look_ahead_distance,0])*np.sin(psi_desired)
