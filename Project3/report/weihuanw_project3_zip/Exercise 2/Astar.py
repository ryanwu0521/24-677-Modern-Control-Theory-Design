...
class AStar(object):
    ...
    def plan(self, start_ind, goal_ind):
        ...
        while len(open_list):
            ...
            # get and add current node to the closed list
            ...
            for successor in self.get_successor(current):
                ...
                successor.g_value = current.g_value + 1
                successor.h_value = self.heuristic(successor, goal_node)
                successor.f_value = successor.g_value + successor.h_value

                if tuple(successor.pose) in closed_list:
                    continue

                in_open_list = any(successor == node for node in open_list)

                if not in_open_list or successor.g_value < current.g_value:
                    if in_open_list:
